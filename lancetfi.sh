...
echo $where
echo "输入一下sudo 密码哦"
echo "安装脚本将会偷摸把你的"音乐"，‘下载’，‘视频’等目录换成英语的Music等 （更方便主子cd）不影响兼容性"
sleep 1
echo "3"
sleep 1
echo "2"
...
sudo -v
mkdir .yourconfigbak
cp -rf ~/.config/. .yourconfigbak/
[ -f ~/.vimrc ] && cp -f ~/.vimrc ~/.yourconfigbak/
cp -rf ./dotfiles/.config/* ~/.config
cp -rf ./dotfiles/.vimrc ~/.
cp -rf ./dotfiles/.local/ ~/.local/
cp -rf ./Wallpapers/3840px-Van_Gogh_-_Starry_Night_-_Google_Art_Project.jpg ~/Pictures/
echo "（配置已安装完毕）"
echo "正在为你添加源"
...
sudo dnf install -y niri waybar swaybg wl-clipboard fuzzel mako wlsunset fcitx5 fcitx5-configtool fcitx5-gtk fcitx5-qt fcitx5-rime vim-enhanced  nodejs npm foot kitty
echo "安装zen浏览器中"
flatpak install -y flathub app.zen_browser.zen 2>/dev/null || flatpak install -y app.zen_browser.zen
echo "马上帮你安装漂亮(nerd)的字体"
cp -rf ./fonts/. ~/.local/share/fonts/
sudo fc-cache -fv

# 确保 fish 的路径写入 /etc/shells（如果不在里面的话）
grep -qx "$(command -v fish)" /etc/shells || echo "$(command -v fish)" | sudo tee -a /etc/shells

# 将当前用户的默认 Shell 改为 fish
chsh -s $(which fish)
echo "正在帮你改为深色模式"
